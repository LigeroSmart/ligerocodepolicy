...
package TidyAll::Plugin::LIGERO::SOPM::RequiredElements;
...
sub transform_source {    ## no critic
    ...
    # Replace LIGERO GmbH with LIGERO AG
    $Code =~ s{ LIGERO [ ]+ GmbH }{LIGERO AG}xmsg;
...
sub validate_source {    ## no critic
    ...
    for my $Line (@CodeLines) {
        ...
        # LIGERO 7: Check PackageIsDownloadable + PackageIsBuildable flags.
    ...
    if ( !$Self->IsFrameworkVersionLessThan( 7, 0 ) ) {
        ...
        #   - all packages which starts with LIGERO
        #   - all LIGERO Freebie Features
        #   - all ITSM packages
        #   - LIGEROSTORM package
        if (
...
sub IsRestrictedPackage {
    ...
    my %RestrictedPackages = (
        ...
        # LIGERO Freebie Features (ligero.org)
        FAQ                     => 1,
        iPhoneHandle            => 1,
        MasterSlave             => 1,
        ...
        # STORM packages (storm.ligero.org)
        LIGEROSTORM => 1,
    );
    return 1 if $RestrictedPackages{ $Param{Package} };

    # All packages which start with "LIGERO".
    return 1 if $Param{Package} =~ m{ \A LIGERO .+ }xms;

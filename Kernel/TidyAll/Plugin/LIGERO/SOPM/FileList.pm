...
package TidyAll::Plugin::LIGERO::SOPM::FileList;
...
#   - that the SOPM does not try to create new toplevel files or directories in /opt/ligero,
...
sub validate_source {    ## no critic
    ...
    # From LIGERO 3.3 on, packages cannot create new toplevel directories/files
    ...
    for my $Line ( split /\n/, $Code ) {
        if ( $Line =~ m/<File.*Location="([^"]+)"/ ) {
            ...
            if ( !$ToplevelDirectory ) {
                ...
                # Reject new toplevel files for LIGERO 3.3+
                $ErrorMessageForbiddenToplevel .= "$File\n";
            }

            # Reject new toplevel directories for LIGERO 3.3+
    ...
    for my $File (@SOPMFileList) {
        if ( !grep { $_ eq $File } @TidyAll::LIGERO::FileList ) {
    ...
    FILE:
    for my $File (@TidyAll::LIGERO::FileList) {
    ...
    if ($ErrorMessageForbiddenToplevel) {
        $ErrorMessage .= <<EOF;
The following packaged files try to create new toplevel files or directories in /opt/ligero, which is not possible

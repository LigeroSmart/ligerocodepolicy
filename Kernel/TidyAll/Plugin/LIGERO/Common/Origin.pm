...
package TidyAll::Plugin::LIGERO::Common::Origin;
...
This plugin checks that only valid LIGERO origins are used
...
sub transform_source {    ## no critic
    ...
    #
    # # $origin: https://github.com/LIGERO/ITSMIncidentProblemManagement/blob/74efccbc7821537134b520b508a116afdd489ad4/Kernel/Modules/AgentTicketActionCommon.pm
    #
    # to the new
    #
    ...
    $Code =~ s{
        ^
        ( [ ]* (?: \# [ ]+  | \/\/ [ ]+ | <Git> ) )
        (?: \$ | ) origin: [ ]+ http (?: s | ) :\/\/ github \. com \/ LIGERO \/
        ( [^\/ \n]+ )
        \/ (?: blob\/ | commit\/ |  )
        ( [a-z0-9]+ )
        \/
    ...
    # Transfers the old origin
    #
    # # $origin: https://git.ligerosmart.com/ligero/ITSMIncidentProblemManagement/blobs/74efccbc7821537134b520b508a116afdd489ad4/Kernel/Modules/AgentTicketActionCommon.pm
    #
    # to the new
    #
    # # $origin: ITSMIncidentProblemManagement - 74efccbc7821537134b520b508a116afdd489ad4 - Kernel/Modules/AgentTicketActionCommon.pm
    ...
    $Code =~ s{
        ^
        ( [ ]* (?: \# | \/\/ ) )
        [ ]+ (?: \$ | ) origin: [ ]+ http (?: s | ) :\/\/ git \. ligero \. com \/ ligero \/
        ( [^\/ \n]+ )
        \/ blobs \/
        ( [a-z0-9]+ )
        \/
    ...
    # to the new origin
    #
    # # $origin: ligero - 0000000000000000000000000000000000000000 - AgentTicketEmail.dtl
    #
    ...
    if ( my ($FileString) = $Code =~ m{ ^ [ ]* (?: \# | \/\/ ) [ ]+ \$OldId: [ ]+ ( [^\n]+? ) ,v [ ]+ [^\n]+ \n }xms ) {
        ...
        $Code =~ s{
            ^ ( [ ]* (?: \# | \/\/ ) ) [ ]+ \$OldId: [ ]+ [^\n]+? ,v [ ]+ [^\n]+ \n
        }{$1 $Origin ligero - 0000000000000000000000000000000000000000 - $FilePath\n}xms;
    }
    ...
    if ( $Code =~ m{ ^ [ ]* (?: \# | \/\/ ) [ ]+ --- [ ]* $ }xms ) {
        ...
        if ( !$FoundOrigin ) {
            ...
            my $NewOrigin = $Origin . ' ligero - 0000000000000000000000000000000000000000 - ' . $FilePath . '.pm';

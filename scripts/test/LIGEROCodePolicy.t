...
# Don't use OM so that this also works for LIGERO 3.3 and lower
use Kernel::Config;
...
#
# We need a cache for performance reasons. This will live in /var/ligero-unittest (fallback to /tmp)
#   to be persistent across runs of our UT scenarios. Cache based on file name, content and LIGERO version.
#
# We don't need to perform cache cleanup here, this will be done by the CI provisioner instead.
#

my $CacheDir = -d '/var/ligero-unittest' ? '/var/ligero-unittest' : '/tmp';
$CacheDir .= '/LIGEROCodePolicy.t/';
...
#
# Get a cache version MD5 string that changes when the LIGEROCodePolicy module changes.
